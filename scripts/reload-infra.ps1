...
kubectl delete -f "$scriptRoot/../k8s/overlays/dev-build.yaml"
...
if (!$Clean.IsPresent) {
    ...
    kustomize build "$scriptRoot/../k8s/overlays/dev" --enable-helm > "$scriptRoot/../k8s/overlays/dev-build.yaml"

    write-host "Recreating the local build configuration..." -ForegroundColor Green
    kubectl apply -f "$scriptRoot/../k8s/overlays/dev-build.yaml"
